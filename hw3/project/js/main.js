...
class ProductItem extends Item {
    constructor(product, img = 'https://picsum.photos/seed/random/200') {
        super(
    ...
    render() {
        return `<div class="product-item" data-id="${this.id_product}">
                <img src="${this.img}" alt="productImg">
                <div class="desc">
    ...
    addToCartButtonHandler(cart) {
        ...
        document.querySelector(`.product-item[data-id="${this.id_product}"]`)
    ...
    _getCartItems() {
        return fetch(`${API}/getBasket.json`)
            .then(response => response.json()).then(data => {
                ...
                this.cartItemList = [...data.contents].map(product => 
                    this._product2CartItem(product));
            })
            .catch(error => {
                console.log(error);
            });
    ...
    _addCartButtonHandler() {
        document.querySelector('.cart__btn').addEventListener('click', () => {
            let isVisible = (vElement) => {
                return !(vElement.offsetWidth === 1 && vElement.offsetHeight === 1)
...
class CartItem extends Item{
    constructor(product, img = 'https://picsum.photos/seed/random/60') {
        super(
            product.product_name,
            product.price,
            product.id_product,
